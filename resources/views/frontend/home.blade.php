...
@section('content')
    ...
    <script>
        ...
        function enableDragScroll(track) {
            ...
            track.addEventListener('mouseleave', () => { isDown = false; track.style.cursor = 'default'; });
            track.addEventListener('mouseup', () => { isDown = false; track.style.cursor = 'default'; });
        ...
        document.getElementById('latestPrev').addEventListener('click', () => latestTrack.scrollBy({ left: -280, behavior: 'smooth' }));
        document.getElementById('latestNext').addEventListener('click', () => latestTrack.scrollBy({ left: 280, behavior: 'smooth' }));
        enableDragScroll(latestTrack);

        /* ── Latest Offers nav ── */
        const offersTrack = document.getElementById('offersTrack');
        document.getElementById('offersPrev').addEventListener('click', () => offersTrack.scrollBy({ left: -280, behavior: 'smooth' }));
        document.getElementById('offersNext').addEventListener('click', () => offersTrack.scrollBy({ left: 280, behavior: 'smooth' }));
        ...
        (function () {
            ...
            function goTo(next, dir) {
                ...
                current = next;
                setTimeout(() => { slides[prev].classList.remove(exitClass); busy = false; }, 470);
            }

            function advance(dir) { goTo((current + dir + slides.length) % slides.length, dir); }
            function resetTimer() { clearInterval(timer); timer = setInterval(() => advance(1), 5000); }

            prevBtn.addEventListener('click', () => { advance(-1); resetTimer(); });
            nextBtn.addEventListener('click', () => { advance(1); resetTimer(); });
            dots.forEach(dot => {
            ...
            slider.addEventListener('touchstart', e => { tcsStartX = e.touches[0].clientX; }, { passive: true });
